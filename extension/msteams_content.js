...
function showNotificationLive() {
    ...
    yesButton.addEventListener("click", () => {
      const meetingId = chrome.storage.local.get("meetingId");
...
function setupWebSocket(meetingId) {
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        const wsUrl = `wss://${BASE_URL_BACKEND.replace('https://', '')}/ws?meeting_id=${meetingId}&user_id=${userId}`;
...
async function fetchLateMeetingRecap() {
    ...
    chrome.storage.local.get("meetingId", async (result) => {
        ...
        chrome.runtime.sendMessage(
            ...
            function(response) {
                if (response.success && response.data) {
                    console.log("Late meeting recap fetched successfully:", response.data);
...
async function checkTeamsMeetingStart() {
    ...
    if (meetingStartIndicator && !captionsActivated) {
        ...
        try {
            ...
            const meetingInfoButton = await waitForElement("meeting-info-button", 1000);
        ...
        if (transcriptMessages.length > 0) {
            ...
            if (ws && ws.readyState === WebSocket.OPEN) {
                ...
                console.log("WebSocket not connected - couldn't send transcript update");
            }
            ...
            chrome.runtime.sendMessage({
                ...
                if (chrome.runtime.lastError) {
                    ...
                    showNotificationLive({
                        status: 200,
                        message: "Meeting has started. Need a summary of what has been said?",
                    });
                    return;
                }
