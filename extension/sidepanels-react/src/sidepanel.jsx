...
const Sidepanel = () => {
  ...
  useEffect(() => {
    ...
    async function fetchAINotes() {
      try {
        // Show loading state
        setSummaryContent('<div class="loading">Generating meeting notes...</div>');
        ...
        if (!result.transcript || result.transcript.length === 0) {
          setSummaryContent("<p>No transcript available to generate notes.</p>");
        ...
        fetch(`${AMUREX_CONFIG.BASE_URL_BACKEND}/end_meeting`, {
          ...
          .catch((error) => {
            ...
            setSummaryContent("<p>Failed to generate meeting notes. Please try again later.</p>");
            setActionItems("<p class='error-details'>Error: Failed to process server response</p>");
          });
      } catch (error) {
      ...
      <div id="auth-container" className="auth-container" style={{ display: isAuthenticated ? 'none' : 'block' }}>
        <div className="auth-content">
          ...
          <div className="auth-buttons">
            <button id="sign-in-btn" className="sign-in-btn" onClick={handleSignIn}>Sign In</button>
            <button id="sign-up-btn" className="sign-up-btn" onClick={handleSignUp}>Sign Up</button>
          </div>
        </div>
      </div>

      <div id="main-container" className="main-container" style={{ display: isAuthenticated ? 'block' : 'none' }}>
        <header className="header">
          <div className="header-content">
            <div className="logo-container">
              <img src="assets/logo.svg" alt="Amurex Logo" className="logo" />
            </div>
            <div className="header-actions">
              <button id="close-btn" className="close-btn" onClick={handleCloseSidebar}>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M1 1L13 13M1 13L13 1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                </svg>
              </button>
              <button id="previous-transcripts" className="previous-transcripts-btn" onClick={handlePreviousTranscripts}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
              ...
              <button id="settings-btn" className="settings-btn" onClick={handleSettings}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
        ...
        <div className="content-wrapper">
          <div className="section">
            <div className="section-header">
              ...
              <div className="section-actions">
                <button className="action-btn copy-btn" onClick={() => handleSectionCopy(summaryRef)}>
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                ...
                <button className="action-btn edit-btn" onClick={() => handleSectionEdit(summaryRef)}>
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
          ...
          <div className="section">
            <div className="section-header">
              <h3>Action Items</h3>
              <div className="section-actions">
                <button className="action-btn copy-btn" onClick={() => handleSectionCopy(actionItemsRef)}>
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                ...
                <button className="action-btn edit-btn" onClick={() => handleSectionEdit(actionItemsRef)}>
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
          ...
          <div className="section" style={{ display: 'none' }}>
            <h2 style={{ color: "rgba(255, 255, 255, 0.6)" }}>Email this summary</h2>
            <div id="email-actions" className="email-actions card">
              <div className="email-input-container">
                ...
                />
                <button id="add-email" className="add-btn" onClick={handleAddEmail}>Add</button>
              </div>
              <div className="email-list" id="email-list">
                {emailList.map((email, index) => (
                  <div key={index} className="email-item">
                    <span>{email}</span>
                    <button className="remove-email" onClick={() => handleRemoveEmail(index)}>×</button>
                  </div>
                ))}
              </div>
              <button 
